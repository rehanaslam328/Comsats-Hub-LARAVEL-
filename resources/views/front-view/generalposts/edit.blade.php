...
@section('front-master')
    ...
    <div class="col-md-7">
        ...
        <form action="{{route('generalupdate')}}" method="post" enctype="multipart/form-data" >
            @csrf
            <input name="id" type="text" value="{{$post->id}}"   class="form-control" placeholder="Write what you wish" required>
            <input name="title" type="text" value="{{$post->post}}"   class="form-control" placeholder="Write what you wish" required>
